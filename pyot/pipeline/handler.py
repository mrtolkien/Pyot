    ...
    def validate_handler(self, handler: Dict):
        for code, c in handler.items():
            if code not in self.DEFAULT_HANDLER:
                raise RuntimeError(
                    f"Handler code {code} is not a valid status for error handling"
                )
            strategy = c[0]
            try:
                params = c[1]
                ...
                raise AttributeError(
                    "Handler values receives 2 paramameters, 1 was given"
                ) from e
            for a in params:
                if not isinstance(a, int):
                    raise RuntimeError(
                        f"Handler strategy takes 'int' parameters, '{type(a)}' was given"
                    )
            if strategy == "T":
                if len(params) != 0:
                    raise RuntimeError(
                        f"Handler strategy 'T' (Throw) takes 0 parameters, but {len(params)} was given"
                    )
            elif strategy == "E":
                if len(params) != 2:
                    raise RuntimeError(
                        f"Handler strategy 'E' (Exp. backoff) takes 2 parameters, but {len(params)} was given"
                    )
            elif strategy == "R":
                if len(params) != 1:
                    raise RuntimeError(
                        f"Handler strategy 'R' (Retry) takes 1 parameters, but {len(params)} was given"
                    )
            else:
                raise RuntimeError(
                    f"Handler strategy '{strategy}' is not a valid token for error handling"
                )
    ...
    async def consume(self, code: int, origin: str):
        ...
        if self.exception is None:
            if code == 404:
                ...
                self.exception = Exception(
                    "Unexpected error. Please open an issue on github repository"
                )
