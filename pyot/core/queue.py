...
class Queue:
    ...
    def __init__(
        ...
        if workers < 1:
            raise ValueError("Number of workers must be an integer greater than 0")
    ...
    async def __aenter__(self) -> "Queue":
        ...
        LOGGER.log(
            self.log_level, f"[Trace: Pyot Queue] Spawned {self.workers_num} workers"
        )
        return self
    ...
    async def __aexit__(self, *args):
        ...
        LOGGER.log(
            self.log_level, f"[Trace: Pyot Queue] Joined {self.workers_num} workers"
        )
        return
