...
class CDragon(Store):
    ...
    def __init__(
        self, game: str, error_handler: Dict[int, Tuple] = None, log_level: int = 0
    ):
        self.game = game
    ...
    async def get(
        self, token: PipelineToken, session: aiohttp.ClientSession, **kwargs
    ) -> Dict:
        ...
        while error_token.allow():
            try:
                ...
                LOGGER.log(
                    self.log_level, f"[Trace: {self.game} > CDragon] GET: {token.value}"
                )
            except Exception:
                response = None
