...
class MongoDB(Store):
            ...
            if "setAt_1" in indexes:
                if self.expirations.get_timeout(method) >= 0:
                    await data.command(
                        ...
                        index={
                            "keyPattern": {"setAt": 1},
                            "expireAfterSeconds": self.expirations.get_timeout(method),
                        },
                    )
                else:
                ...
                await data[method].create_index(
                    "setAt", expireAfterSeconds=self.expirations.get_timeout(method)
                )
            else:
                await data[method].create_index("token")
        return data
    ...
    async def set(self, token: PipelineToken, value: Any, **kwargs) -> None:
        ...
        if timeout != 0:
            ...
            LOGGER.log(
                self.log_level,
                f"[Trace: {self.game} > MongoDB > {self.location}] SET: {token.value}",
            )
    ...
    async def get(self, token: PipelineToken, **kwargs) -> Any:
        ...
        LOGGER.log(
            self.log_level,
            f"[Trace: {self.game} > MongoDB > {self.location}] GET: {token.value}",
        )
    ...
    async def delete(self, token: PipelineToken, **kwargs) -> None:
        ...
        LOGGER.log(
            self.log_level,
            f"[Trace: {self.game} > MongoDB > {self.location}] DELETE: {token.value}",
        )
    ...
    async def clear(self, **kwargs):
        ...
        LOGGER.log(
            self.log_level,
            f"[Trace: {self.game} > MongoDB > {self.location}] CLEAR: Store has been cleared successfully",
        )
