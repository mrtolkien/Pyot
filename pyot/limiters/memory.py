...
class MemoryLimiter(BaseLimiter):
    ...
    async def get_token(self, server: str, method: str):
        ...
        async with self.lock:
            ...
            now = epoch = time()
            for prefix, i, type in (
                (app_prefix, 0, "app"),
                (app_prefix, 1, "app"),
                (method_prefix, 0, "method"),
                (method_prefix, 1, "method"),
            ):
                prefix_i = f"{prefix}:{i}"
    ...
    async def sync_rates(
        self, token: LimiterToken, response: ClientResponse
            ...
            async with self.lock:
                ...
                for (prefix_i, type, i) in token.pinging:
                    if i >= len(header[f"{type}_limit"]):
                        ...
                        self.entries.set(
                            f"{prefix_i}:maxcall", header[f"{type}_limit"][i][0]
                        )
                        self.entries.set(
                            f"{prefix_i}:timespan", header[f"{type}_limit"][i][1]
                        )
                        self.entries.set(
                            f"{prefix_i}:called", header[f"{type}_count"][i][0]
                        )
                        self.entries.set(
                            f"{prefix_i}:rollover",
                            self.entries.get(f"{prefix_i}:flying"),
                        )
